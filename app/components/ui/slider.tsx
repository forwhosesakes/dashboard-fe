...
>(({ className, ...props }, ref) => (
  <SliderPrimitive.Root
    ...
    {...props}
  >
    <SliderPrimitive.Track className="relative h-1.5 w-full grow overflow-hidden rounded-full bg-[#E9EAEB]">
      <SliderPrimitive.Range className={
        
        cn(
             "absolute h-full bg-secondary disabled:bg-muted",
   ...
   { !props.disabled&& <span className="absolute bottom-4 shadow-custom border border-tertiary-500 rounded-lg px-2 py-3 text-xs bg-white">{props.value}%</span>}
